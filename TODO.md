...
### Output & hardware guidance

- [x] **Memory budget column** — each format row in the README download table now includes an "Est. RAM" column computed as `domains × 50 bytes` (MikroTik empirical figure); generated by `post_run.py` so all tier tables stay in sync automatically

- [ ] **Categorized outputs** — separate compiled lists per content category so users can compose their own stack (MikroTik AdList supports multiple URLs). Proposed categories:

  | Category | What it covers | Example sources |
  |---|---|---|
  | `ads` | Advertising networks, ad servers | HaGeZi Pro, OISD, Peter Lowe, AdGuard filter |
  | `trackers` | Analytics, metrics, fingerprinting | HaGeZi Pro, StevenBlack, WindowsSpyBlocker |
  | `malware` | Malware, ransomware, phishing, scam | TIF, URLHaus, mitchellkrogza, Spam404, DurableNapkin |
  | `bypass` | DoH/VPN/proxy bypass services | cbuijs DoH, cbuijs DoH+VPN+proxy |
  | `smart-tv` | Smart TV telemetry & tracking | Perflyst SmartTV |
  | `cryptomining` | Browser-based crypto mining | NoCoin |
  | `regional` | Country/language-specific lists | (not currently sourced) |

  **Design decisions to resolve before implementing:**
  - Add `category=` tag to `sources.conf` alongside `tier=` — most sources will need multiple categories (e.g. `category=ads,trackers`)
  - Policy for cross-category domains: a domain seen in both an ads source and a malware source appears in **both** category files
  - Category outputs are subsets of their tier — a domain only appears in `processed/good/malware/` if it's in the good tier and came from a malware-tagged source
  - Most current sources are mixed ads+trackers; meaningful per-category separation requires manually reviewing and tagging all sources, and accepting that `ads` and `trackers` will heavily overlap
  - Output path: `processed/<tier>/categories/<category>/blocklist.txt` (all six formats per category per tier) — or simpler, just the two most useful formats (hosts + domains) since that covers MikroTik + generic resolvers
  - README and `post_run.py` need category-aware sections
